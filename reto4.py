...
import time
import random
#cambiamos la forma de importación de la librería math para no escribir tantas veces math.funcion
...
distanciaparatiempo=None #declaramos una nueva variable global para guardar la distancia final
...
def ImprimirRestaurantesFav(listaoriginal):
    ...
    opcion=int(input("Por favor seleccione su ubicacion actual: "))
...
def ImprimirMensajeCercanias(min1,min2, basededatos,listadistancias ):
    ...
    global distanciaparatiempo #declaramos la variable global
    
    if basededatos[min1][2] > basededatos[min2][2]:
        
        print(f"1. La zona WIFI mas cercana en la latitud: '{basededatos[min1][0]}' longitud: '{basededatos[min1][1]}', está a {listadistancias[min1]} metros, y tiene {basededatos[min1][2]} platos.")
        print(f"2. La segunda zona WIFI más cercana está en la latitud: '{basededatos[min2][0]}' longitud: '{basededatos[min2][1]}', está a {listadistancias[min2]} metros, y tiene {basededatos[min2][2]} platos.")
        opcdestino=int(input("Por favor seleccione la zona WIFI a la cual desea ir, para recibir indicaciones: "))
        if opcdestino==1: 
            distanciaparatiempo = listadistancias[min1] #asignamos la distancia final al valor elegido por el usuario
        ...
        print(f"1. La zona WIFI mas cercana en la latitud: '{basededatos[min2][0]}' longitud: '{basededatos[min2][1]}', está a {listadistancias[min2]} metros, y tiene {basededatos[min2][2]} platos.")
        print(f"2. La segunda zona WIFI más cercana está en la latitud: '{basededatos[min1][0]}' longitud: '{basededatos[min1][1]}', está a {listadistancias[min1]} metros, y tiene {basededatos[min1][2]} platos.")
        opcdestino=int(input("Por favor seleccione la zona WIFI a la cual desea ir, para recibir indicaciones: "))
        if opcdestino==1:
            distanciaparatiempo= listadistancias[min2]
            DarIndicaciones(restauranteactual,basededatos[min2])
...
def CalcularTiempoRecorrido():
    ...
    if distanciaparatiempo==0: #Si la distancia es 0 no hacemos nada
        pass
        
    else:
        auto=distanciaparatiempo/20.83 #en caso contrario calculamos el tiempo que se demorará el recorrido
        moto=distanciaparatiempo/19.44
        
        if auto > 60: #en caso de ser más de 60 segundos convertimos a minutos 
            auto=auto/60
            tiempo1="minutos"
        ...
        moto=round(moto,2) #hacemos un redondeo a 2 cifras
        auto=round(auto,2)
...
if ValidacionDatos(UsuarioGuardado,UsuarioIngresado): 
    if ValidacionDatos(input("Ingrese su contraseña: "), ClaveGuardada): 
        ...
        if ValidacionDatos(captcha,verificacion): 
            ...
            while contadorerrores<5:
                listadistancias=[] #Reiniciamos la lista de distancias dentro del while
                os.system("cls")
                ImprimirLista()
                ...
                if opcionelegida > 0 and opcionelegida < 8:
                    ...
                    if opcionelegidalista==opc1:
                        ...
                        if listacoordenadas==[]:
                            ...
                            ImprimirCoordenads(listacoordenadas)#Arreglamos la lista que llamamos en las funciones
                        
                    elif opcionelegidalista==opc3:

                        MostrarRestaurantesFav(listacoordenadas) #Arreglamos la lista que llamamos en las funciones
    ...
    ErrorConMensaje("Este fue mi primer programa y vamos por más")
